...
class CnnOcr:
    ...
    def cnnNet(self):
        ...
        """
        weight = {
            # 输入 128*64*3   180*100*3

            # 第一层
            'wc1_1': tf.get_variable('wc1_1', [5, 5, 3, 32]),  # 卷积 输出：128*64*32    180*100*32
            'wc1_2': tf.get_variable('wc1_2', [5, 5, 32, 32]),  # 卷积 输出：128*64*32   180*100*32
            # 池化 输出：64*32*32    90*50*32

            # 第二层
            'wc2_1': tf.get_variable('wc2_1', [5, 5, 32, 64]),  # 卷积 输出：64*32*64    90*50*64
            'wc2_2': tf.get_variable('wc2_2', [5, 5, 64, 64]),  # 卷积 输出：64*32*64    90*50*64
            # 池化 输出：32*16*64    45*25*64

            # 第三层
            'wc3_1': tf.get_variable('wc3_1', [3, 3, 64, 64]),  # 卷积 输出：32*16*256   45*25*256
            'wc3_2': tf.get_variable('wc3_2', [3, 3, 64, 64]),  # 卷积 输出：32*16*256   45*25*256
            # 池化 输出：16*8*256    23*13*256

            # 第四层
            'wc4_1': tf.get_variable('wc4_1', [3, 3, 64, 64]),  # 卷积 输出：16*8*64 23*13*64
            'wc4_2': tf.get_variable('wc4_2', [3, 3, 64, 64]),  # 卷积 输出：16*8*64 23*13*64
            # 池化 输出：8*4*64 12*7*64

            # 全链接第一层
            'wfc_1': tf.get_variable('wfc_1', [4224 * 64, 2048]),
